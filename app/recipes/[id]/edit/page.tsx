...
export default function EditRecipePage() {
  ...
  const { customRecipes, updateCustomRecipe } = useGeneratorStore();
  ...
  if (!recipe || !form) {
    return (
      <div className="min-h-screen bg-brand-bg p-6">
        <p className="text-brand-text">Recipe not found or not editable.</p>
        <Link href="/recipes" className="mt-3 inline-block text-brand-primary underline">
          Back to recipes
        </Link>
      </div>
    );
  ...
  const save = () => {
    ...
    updateCustomRecipe(recipe.id, { ...payload, imageUrl });
    router.push("/recipes");
  };
  ...
  return (
    <div className="min-h-screen bg-brand-bg">
      ...
      <main className="mx-auto w-full max-w-[960px] px-4 py-8 md:px-8">
        <h1 className="text-2xl font-semibold text-brand-text">Edit recipe</h1>
